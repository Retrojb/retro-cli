...
import ora from 'ora';
import { DirectoryConflictError, TimeoutError, ScaffoldError, } from '../types.js';
export async function cloneTemplate(config) {
    ...
    try {
        ...
        throw new ScaffoldError(`Failed to clone template: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
