...
export async function configureProject(config) {
    ...
    if (!isValidNpmPackageName(config.projectName)) {
        throw new ScaffoldError(`Invalid npm package name: "${config.projectName}"`);
    }
    ...
    if (config.additionalOptions.includes('jest')) {
        ...
        await fs.writeFile(jestConfigPath, "export default { testEnvironment: 'node' };\n", 'utf-8');
    }
    ...
    if (config.additionalOptions.includes('eslint')) {
        ...
        await fs.writeFile(eslintConfigPath, 'export default [{ rules: {} }];\n', 'utf-8');
    }
